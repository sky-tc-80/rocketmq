...
public class UnpooledDirectByteBuf extends AbstractReferenceCountedByteBuf {
    ...
    public ByteBuf capacity(int newCapacity) {
        ...
        if (newCapacity > oldCapacity) {
            ...
            // 将新缓存区的 postion,limit 属性设置为0，老缓存区 limit
            newBuffer.position(0).limit(oldBuffer.capacity());
            // 将原缓冲区写入到新的缓存区，然后将缓存区置的 position 设置为0，limt 设置为 capacity,
            // 其实这里设置position,capacity 的意义不大，因为 ByteBuf 并不会利用内部的 ByteBuffe r的 limit,postion 属性，
            // 而是使用readerIndex, wriateIndex
            newBuffer.put(oldBuffer);
            // 关联新的 ByteBuffer, 并释放原缓存区的空间
            newBuffer.clear();
            setByteBuffer(newBuffer);
        }
        // 压缩缓存区。实现思路是新建一个缓存区，如果 readerIndex 大于新建的 ByteBuffer 的 capacity,则无需将旧的缓存区内容写入到新的缓存区中。
        // 如果 readerIndex 小于新 capacity,那需要将 readerIndex 至(  Math.min(writerIndex, newCapacity) )
        // 直接的内容写入到新的缓存，然后释放旧的缓存区
