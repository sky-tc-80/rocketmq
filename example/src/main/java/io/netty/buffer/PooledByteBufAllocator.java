...
public class PooledByteBufAllocator extends AbstractByteBufAllocator {
    ...
    /**
     * PoolArena 的个数，从下文的初始化中可以看出，取值为 CPU 核心逻辑线程数与 （最大运行内存【堆内存或堆外内存】的大小的二分之一 除以每个 Chunk-size,
     * 并除以3，是确保每个 PoolArena 包含三个 Chunk）
     */
    ...
    */
    public PooledByteBufAllocator(boolean preferDirect, int nHeapArena, int nDirectArena, int pageSize, int maxOrder,
        ...
        // 比如pageSize为8192, 2的13次方法，那pageShifts为Integer.SIZE - 1  - Integer.numberOfLeadingZeros(pageSize) = 13，
        // 其中Integer.numberOfLeadingZeros返回高位连续的0的个数，8192为2的13次方，所有8192的二进制的高位有18个0，所以pageShifts为13
        // pageShifts 在计算 numSmallPoolSubpages 大小是用到，numSmallPoolSubpages 是数组 smallPoolSubpages[] 的长度，
        // 因为Netty 中关于 small 内存的大小定义是  [1024,pageSize), 而且是翻倍存放，比如 1024 2048 4096等，该值在讲解 PoolArena 数据结构时还会详细介绍
    ...
    protected ByteBuf newHeapBuffer(int initialCapacity, int maxCapacity) {
        ...
        // 初始化一个PoolArena[]数组，每个线程轮询PoolArena数组，在整个线程运行过程中，只会与一个特定的PoolArena打交道。
