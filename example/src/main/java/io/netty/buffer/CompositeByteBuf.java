...
/**
 * 组合ByteBuf，比如一个协议分为协议头，协议主体，这两部分都是一个单独的 ByteBuf, 在处理这两个ByteBuf 时，然后需要将这个数据在网络上传输。
 * 常规做法，是需要创建一个更大的 ByteBuf, 然后将 head,body 的内容写入新创建的ByteBuf, CompositeByteBuf 就是用来解决这个，提供一个类似数据库视图这样的概念
