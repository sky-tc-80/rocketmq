...
class PooledHeapByteBuf extends PooledByteBuf<byte[]> {
    ...
    static PooledHeapByteBuf newInstance(int maxCapacity) {
        // 在这里首先先知道是Netty关于ByteBuf的一个轻量级的对象池的实现，Netty会在本地线程变量中重复利用释放掉的ByteBuf，
        // 关于本地线程缓存机制，请看下文关于本地线程内存分配专题讲解。创建一个PooledByteBuf对象后，
        // 然后进入到PoolArena.allocate方法，为创建的PooledByteBuf对象分配内存（PooledByteBuf内部缓存区的）
