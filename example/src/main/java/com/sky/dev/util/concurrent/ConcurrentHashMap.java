...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    private final void addCount(long x, int check) {
        ...
        if ((as = counterCells) != null ||
            ...
            if (as == null || (m = as.length - 1) < 0 ||
                (a = as[com.sky.dev.util.concurrent.ThreadLocalRandom.getProbe() & m]) == null ||
                !(uncontended =
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        int h;
        if ((h = com.sky.dev.util.concurrent.ThreadLocalRandom.getProbe()) == 0) {
            com.sky.dev.util.concurrent.ThreadLocalRandom.localInit();      // force initialization
            h = com.sky.dev.util.concurrent.ThreadLocalRandom.getProbe();
            wasUncontended = true;
        }
        ...
        for (;;) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                h = com.sky.dev.util.concurrent.ThreadLocalRandom.advanceProbe(h);
            }
     ...
     * TreeBin 并不存储实际数据,它维护对桶内红黑树的读写锁
