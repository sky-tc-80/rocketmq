...
import com.sky.dev.util.concurrent.LinkedBlockingQueue;
import com.sky.dev.util.concurrent.SynchronousQueue;
...
import com.sky.dev.util.concurrent.atomic.AtomicInteger;
 ...
 * @since 1.5
 */
public class ThreadPoolExecutor extends com.sky.dev.util.concurrent.AbstractExecutorService {
    /**
    ...
    // Worker实现AQS，并自己实现了简单不可重入独占锁，其中 state=0 表示当前锁未被获取状态，state=1 表示锁被获取，
    // state=-1 表示Work创建时候的默认状态，创建时候设置 state=-1 是为了防止 runWorker 方法运行前被中断
     ...
     */
    private volatile com.sky.dev.util.concurrent.RejectedExecutionHandler handler;
    ...
    private final class Worker
         ...
         */
        //state=0表示当前锁未被获取状态，state=1表示锁被获取，
    ...
    public static class CallerRunsPolicy implements RejectedExecutionHandler {
         ...
         */
        public void rejectedExecution(Runnable r, com.sky.dev.util.concurrent.ThreadPoolExecutor e) {
            if (!e.isShutdown()) {
                r.run();
            }
        }
     ...
     * rejected task.
     */
    public static class DiscardPolicy implements com.sky.dev.util.concurrent.RejectedExecutionHandler {
        /**
